...
void GRAPHshow(Graph G) {
    ...
    for (i = 0; i < G->V; i++) {
        printf("%2d:", i);
        for (adjVertex = G->adj[i]; adjVertex != ((void *)0); adjVertex = adjVertex->next) {
...
void dijkstra(Graph G, int source, int weight[]) {
    ...
    while (!PQempty()) {
        item = PQdelmin();
        vertex = item.key;
        if (weight[vertex] != 1000) {
            for (t = G->adj[vertex]; t != ((void *)0); t = t->next)
...
int main() {
  ...
  name.sin_family = 2;
  name.sin_port = ((__uint16_t)(__builtin_constant_p(2610) ? ((__uint16_t)((((__uint16_t)(2610) & 0xff00) >> 8) | (((__uint16_t)(2610) & 0x00ff) << 8))) : _OSSwapInt16(2610)));
